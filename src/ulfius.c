...
#if MHD_VERSION >= 0x00096100
    enum MHD_ResponseMemoryMode mhd_response_flag = ((struct _u_instance *)cls)->mhd_response_copy_data?MHD_RESPMEM_MUST_COPY:MHD_RESPMEM_MUST_FREE;
    enum MHD_Result mhd_ret = MHD_NO;
#else
    int mhd_response_flag = MHD_RESPMEM_MUST_FREE, mhd_ret = MHD_NO;
#endif
  int callback_ret = U_OK, i, close_loop = 0, inner_error = U_OK;
